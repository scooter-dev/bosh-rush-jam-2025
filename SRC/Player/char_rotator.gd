	...
	match player.mode:
		Player.RUNNING:
			if abs(rotMomentum) > 0.01:
				...
				var dir = Vector2(-PlayerInput.fbrl.y,-PlayerInput.fbrl.x).angle() + PI
				var aRt = lerp_angle(dir, baseSpRot, clamp(abs(rotMomentum * 0.5),0,1))
				global_rotation.y = lerp_angle(global_rotation.y, aRt, delta * 6.0)
				#print(rotMomentum)
			elif PlayerInput.fbrl.length_squared() > 0.01:
				global_rotation.y = lerp_angle(global_rotation.y, Vector2(-PlayerInput.fbrl.y,-PlayerInput.fbrl.x).angle() + PI, 4.0 * delta)
